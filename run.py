...
async def run_module_safe(
        account: Account,
        process_func: Callable[[Bot], Coroutine[Any, Any, Any]]) -> Any:
        ...
        try:
            if config.delay_before_start.min > 0:
                if process_func == process_farming and account.email not in accounts_with_initial_delay:
                    ...
                    logger.info(
                        f"账户: {account.email} | 初始farm延迟: {random_delay} 秒")
...
async def run_module(
        ...
        process_func: Callable[[Bot], Coroutine[Any, Any, Any]]) -> tuple[Any]:
...
async def run() -> None:
    ...
    module_map = {
        ...
        "re_verify_accounts":
        (config.accounts_to_reverify, process_re_verify_accounts),
    }
