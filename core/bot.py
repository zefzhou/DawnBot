...
class Bot(DawnExtensionAPI):
    ...
    async def handle_invalid_account(
            email: str, password: str, reason: Literal["unverified",
                                                       "banned"]) -> None:
        if reason == "unverified":
            logger.error(
                f"Account: {email} | Email not verified, run re-verify module | Removed from farming"
            )
            await file_operations.export_unverified_email(email, password)

        else:
            logger.error(
                f"Account: {email} | Account is banned | Removed from farming")
    ...
    async def process_reverify_email(self,
        ...
        try:
            result = await EmailValidator(
                ...
                if not config.redirect_settings.enabled else
                config.redirect_settings.imap_server,
                self.account_data.email if not config.redirect_settings.enabled
                else config.redirect_settings.email, self.account_data.password
                if not config.redirect_settings.enabled else
                config.redirect_settings.password).validate(
                    None if config.redirect_settings.enabled and not config.
                    redirect_settings.use_proxy else self.account_data.proxy)
            if not result["status"]:
                logger.error(
                    f"Account: {self.account_data.email} | Email is invalid: {result['data']}"
                )
            ...
            logger.info(
                f"Account: {self.account_data.email} | Re-verifying email...")
            puzzle_id, answer, task_id = await self.get_captcha_data()

            if not link_sent:
                await self.resend_verify_link(puzzle_id, answer)
                logger.info(
                    f"Account: {self.account_data.email} | Successfully resent verification email, waiting for email..."
                )
                link_sent = True
            ...
            confirm_url = await LinkExtractor(
                ...
                if not config.redirect_settings.enabled else
                config.redirect_settings.password).extract_link(
                    None if config.redirect_settings.enabled and not config.
                    redirect_settings.use_proxy else self.account_data.proxy)

            if not confirm_url["status"]:
                logger.error(
                    f"Account: {self.account_data.email} | Confirmation link not found: {confirm_url['data']}"
                )
                return OperationResult(
                    identifier=self.account_data.email,
                    data=self.account_data.password,
            ...
            if error.error_message in error.BASE_MESSAGES:
                if error.error_message == "Incorrect answer. Try again!":
                    ...
                    logger.warning(
                        f"Account: {self.account_data.email} | Email already exists"
                    )
                    return OperationResult(
                        identifier=self.account_data.email,
                        data=self.account_data.password,
        ...
        try:
            ...
            if error.error_message in error.BASE_MESSAGES:
                if error.error_message == "Incorrect answer. Try again!":
                    ...
                    logger.warning(
                        f"Account: {self.account_data.email} | Most likely email domain <{self.account_data.email.split('@')[1]}> is banned"
                    )
                    return OperationResult(
                        identifier=self.account_data.email,
                        data=self.account_data.password,
    ...
    def get_sleep_until(blocked: bool = False) -> datetime:
        duration = (timedelta(minutes=10) if blocked else timedelta(
            seconds=config.keepalive_interval))
        ...
        try:
            ...
            if db_account_data and db_account_data.session_blocked_until:
                if await self.handle_sleep(
                        db_account_data.session_blocked_until):
                    return
            ...
            if error.error_message == "Email not verified , Please check spam folder incase you did not get email":
                await self.handle_invalid_account(self.account_data.email,
    ...
    async def process_get_user_info(self) -> StatisticData:
        try:
            ...
            if db_account_data and db_account_data.session_blocked_until:
                if await self.handle_sleep(
                        db_account_data.session_blocked_until):
                    return StatisticData(success=False,
                                         referralPoint=None,
                                         rewardPoint=None)
    ...
    async def process_complete_tasks(self) -> OperationResult:
        try:
            ...
            logger.info(
                f"Account: {self.account_data.email} | Completing tasks...")
    ...
    async def login_new_account(self):
        ...
        try:
            ...
            logger.info(
                f"Account: {self.account_data.email} | Successfully logged in")
            ...
            if error.error_message in error.BASE_MESSAGES:
                if error.error_message == "Incorrect answer. Try again!":
                    ...
                    await self.handle_invalid_account(
                        self.account_data.email, self.account_data.password,
                        "unverified")
                    return False
            ...
            await Accounts.set_sleep_until(self.account_data.email,
                                           sleep_until)
            logger.error(
    ...
    async def handle_existing_account(self, db_account_data) -> bool | None:
        ...
        logger.info(
            f"Account: {self.account_data.email} | Using existing session")
        return True
    ...
    async def handle_session_blocked(self):
        ...
        await Accounts.set_session_blocked_until(self.account_data.email,
                                                 sleep_until)
    ...
    async def perform_farming_actions(self):
        try:
            ...
            await Accounts.set_sleep_until(email=self.account_data.email,
