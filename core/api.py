...
class DawnExtensionAPI:
    ...
    def setup_session(self) -> AsyncSession:
        ...
        session.headers = {
            ...
            "en-US,en;q=0.9",
            "origin":
            "chrome-extension://fpdkjdnhkakefebpekbdhillbhonfjjp",
            "priority":
            "u=1, i",
            "user-agent":
            ...
            if "status" in str(response_data):
                if isinstance(response_data, dict):
                    if response_data.get("status") is False:
                        raise APIError(
                            f"API returned an error: {response_data}",
                            response_data)
                ...
                if request_type == "POST":
                    if not url:
                        response = await self.session.post(
                            ...
                            params=params,
                            headers=headers
                            if headers else self.session.headers,
                            cookies=cookies,
                        )
                    else:
                        response = await self.session.post(
    ...
    async def solve_puzzle(
        image: str,
               ...
               | int] | Tuple[str, bool] | Tuple[str, bool, str]:
    ...
    async def register(self, puzzle_id: str, answer: str) -> dict:
        json_data = {
            ...
            "+91",
            "referralCode":
            random.choice(config.referral_codes)
            if config.referral_codes else "",
            "puzzle_id":
            puzzle_id,
            "ans":
            answer,
